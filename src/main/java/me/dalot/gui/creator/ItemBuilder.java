package me.dalot.gui.creator;

import dev.simplix.protocolize.api.item.BaseItemStack;
import dev.simplix.protocolize.api.item.ItemStack;
import dev.simplix.protocolize.data.ItemType;
import me.dalot.gui.creator.enchant.Enchantment;
import me.dalot.gui.creator.enchant.EnchantmentEnum;
...
public class ItemBuilder {
    ...
    public ItemBuilder enchantments(EnchantmentEnum[] enchantment, int level) {
        this.enchantments.clear();
        Arrays.asList(enchantment).stream().forEach(e -> enchantment(e, level));
        return this;
    }

    public ItemBuilder enchantment(EnchantmentEnum enchantment, int level) {
        this.enchantments.add(new Enchantment(enchantment, level));
        return this;
    }

    public ItemBuilder clearEnchantment(EnchantmentEnum enchantments) {
    ...
    public static ItemBuilder from(BaseItemStack item) {
        ...
        if (item.nbtData().get("Enchantments") != null) {
            ...
            for (CompoundTag tag : enchants) {
                builder.enchantment(EnchantmentEnum.valueOf(tag.getString("id").replace("minecraft:", "").toUpperCase()),
                        tag.getInt("lvl"));
            }
        }
        return builder;
