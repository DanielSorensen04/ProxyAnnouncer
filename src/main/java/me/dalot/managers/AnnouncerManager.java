...
public class AnnouncerManager {
    ...
    public static void init() {
        ProxyAnnouncer.getInstance().getServer().getScheduler().buildTask(ProxyAnnouncer.getInstance(), () -> {
            timer++;
            new ArrayList<>(messages).forEach(m -> {
                if (m.getDelay() != null) {
                    if (m.getDelay().getType() == Delay.DelayType.RANGE) {
                        if (timer % m.getDelay().getNext() == 0) {
                            ...
                            ProxyAnnouncer.getInstance().getServer().getAllPlayers().forEach(m::execute);
                        }
                    } else {
